...
If no semver tag exists, the workflow reads `games_theory/__init__.py` and applies the same next-patch rule. The workflow rejects non-semver versions and stops if the selected tag already exists.
